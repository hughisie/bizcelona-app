...
// CONFIRM_HMAC_SECRET must be set in the Vercel dashboard (Settings → Environment Variables).
// Without it, confirm links will still work locally (both sides use empty string consistently)
// but this is not acceptable in production — always set this env var before deploying.
const hmacSecret = process.env.CONFIRM_HMAC_SECRET ?? '';

function makeConfirmUrl(id: string, reply: 'true' | 'false'): string {
  const token = crypto.createHmac('sha256', hmacSecret).update(`${id}:${reply}`).digest('hex');
  return `${APP_URL}/api/connections/confirm?id=${id}&reply=${reply}&token=${token}`;
}

function isAuthorized(request: NextRequest): boolean {
  const cronSecret = process.env.CRON_SECRET;
  const authHeader = request.headers.get('authorization');
  // In Vercel, CRON_SECRET is always set for scheduled functions.
  // Fail open only in local dev where NODE_ENV !== 'production'.
  if (!cronSecret) {
...
export async function GET(request: NextRequest) {
  ...
  for (const connection of connections) {
    ...
    // HMAC-signed URLs — prevents anyone with a bare UUID from flipping reply status
    const yesUrl = makeConfirmUrl(connection.id, 'true');
    const noUrl = makeConfirmUrl(connection.id, 'false');
    ...
    if (emailResult.error) {
      console.error(`Failed to send reminder for connection ${connection.id}:`, emailResult.error);
      errors++;
      continue;
    }

    // Only mark as sent after confirming the email was accepted by Resend.
    // If this update fails, sent stays un-incremented and the cron will retry next run.
    ...
    if (updateError) {
      console.error(`Failed to mark reminder sent for ${connection.id}:`, updateError);
      errors++;
      continue; // Don't increment sent — will retry next cron run
    }
