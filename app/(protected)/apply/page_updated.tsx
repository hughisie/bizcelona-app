...
export default function ApplyPage() {
  ...
  const validateForm = () => {
    ...
    // WhatsApp validation (minimum 9 characters)
    if (formData.whatsappNumber.replace(/[\s-]/g, '').length < 9) {
      errors.whatsappNumber = 'WhatsApp number must be at least 9 characters (include country code like +34...)';
    }
    ...
    // Long-form required fields (minimum length)
    if (formData.whatDoYouDo.trim().length < 20) {
      errors.whatDoYouDo = `Please provide at least 20 characters (currently ${formData.whatDoYouDo.trim().length})`;
    }

    if (formData.hopingToGet.trim().length < 20) {
      errors.hopingToGet = `Please provide at least 20 characters (currently ${formData.hopingToGet.trim().length})`;
    }

    if (formData.hopeToBring.trim().length < 20) {
      errors.hopeToBring = `Please provide at least 20 characters (currently ${formData.hopeToBring.trim().length})`;
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-saffron focus:border-transparent"
              />
              <p className="mt-1 text-sm text-gray-500">{formData.whatDoYouDo.length} characters</p>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-saffron focus:border-transparent"
              />
              <p className="mt-1 text-sm text-gray-500">{formData.hopingToGet.length} characters</p>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-saffron focus:border-transparent"
              />
              <p className="mt-1 text-sm text-gray-500">{formData.hopeToBring.length} characters</p>
