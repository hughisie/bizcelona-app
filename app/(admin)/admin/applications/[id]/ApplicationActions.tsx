...
const WHATSAPP_GROUP_LINK = 'https://chat.whatsapp.com/KbXyAa2jMNHGbt2pDGJP48?mode=gi_t';

const getWelcomeMessage = (name: string, isContributor: boolean) => {
  const sharedOpening = `Dear ${name},
...
Please join our WhatsApp community group here: ${WHATSAPP_GROUP_LINK}

We will be organising a more formal introduction and start sharing our ideas and plans to make this community the best around. Please note, that we are an active community and we want people that will contribute and participate in the discussion and helping others.

We operate a mantra which is "give before you take" so please have this in mind.`;

  const contributorExtra = `
...
See you in the group!`;
  ...
  return sharedOpening + (isContributor ? contributorExtra : '') + sharedClosing;
};

const getWhatsAppLink = (phoneNumber: string, message: string) => {
  // Strip everything except digits — wa.me needs plain international format
...
export default function ApplicationActions({
  ...
  if (application.status === 'approved') {
    ...
    const whatsappLink = getWhatsAppLink(application.whatsapp_number ?? '', welcomeMessage);
      ...
      <div className="space-y-6">
        <div className="bg-green-50 border border-green-200 rounded-lg p-6">
          ...
          <div className="bg-white border border-green-300 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-700">
              An approval email was automatically sent to <span className="font-semibold">{application.email}</span> — it includes the WhatsApp group invite link.
            </p>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h3 className="text-lg font-semibold text-navy mb-1">Send WhatsApp Welcome</h3>
              <p className="text-sm text-gray-600">
                {application.contributor_interest
                  ? 'Message for contributors — includes contributor paragraph'
                  : 'Standard welcome message — includes group invite link'}
              </p>
            </div>
            <div className="flex gap-2">
              <a
                ...
                className="px-4 py-2 rounded-lg font-semibold bg-[#25d366] text-white hover:bg-[#1da851] transition-all duration-200 flex items-center gap-2"
              >
